...
def datasetAllelResponseBuilder(datasetId, referencename, start, alternateBases):
    j = 0
...
def checkParameters(referenceName, start, startMin, startMax, end, endMin, endMax, \
    ...
    if datasetIds:
        for set in datasetIds:
            ...
            datasetAllelResponses.append(datasetAllelResponseBuilder(set, referenceName, start, alternateBases))
    else:
        datasetIds = None
    ...
    if referenceName not in refname or referenceBases not in refbases or assemblyId not in assembly_list or includeDatasetResponses not in datasetresponses:
        #if an error occures the 'exists' must be 'null'
        for set in datasetAllelResponses:
            set['exists'] = None
        bad_request(referenceName, start, startMin, startMax, end, endMin, endMax, \
