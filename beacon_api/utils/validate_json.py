...
async def parse_request_object(request: web.Request) -> Tuple[str, Dict]:
    ...
    if request.method == "GET":
        ...
        if "datasetIds" in items:
            datasetIds: str = request.rel_url.query.get("datasetIds", "")
            items["datasetIds"] = datasetIds.split(",")
            ...
            try:
                ...
                if len(e.path) > 0:
                    ...
                    raise BeaconBadRequest(
                        obj,
                        request.host,
                        f"Provided input: '{e.instance}' does not seem correct for field: '{e.path[0]}'",
                    )
                else:
