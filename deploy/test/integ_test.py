...
DATASET_IDS_LIST = ["urn:hg:1000genome", "urn:hg:1000genome:registered", "urn:hg:1000genome:controlled", "urn:hg:1000genome:controlled1"]
...
async def test_2() -> None:
    ...
    params = {"assemblyId": "GRCh38", "referenceName": "MT", "start": 9, "referenceBases": "T", "alternateBases": "C", "includeDatasetResponses": "HIT"}
    async with aiohttp.ClientSession() as session:
...
async def test_3() -> None:
    ...
    params = {"assemblyId": "GRCh38", "referenceName": "MT", "start": 9, "referenceBases": "T", "variantType": "SNP", "includeDatasetResponses": "HIT"}
    async with aiohttp.ClientSession() as session:
        async with session.get("http://localhost:5050/query", params=params) as resp:
            data = await resp.json()
            if "datasetAlleleResponses" in data and len(data["datasetAlleleResponses"]) > 0:
...
'includeDatasetResponses': 'HIT'}' does not seem correct because: ''referenceName' is a required property'"
    params = {"assemblyId": "GRCh38", "start": 9, "referenceBases": "T", "alternateBases": "C", "includeDatasetResponses": "HIT"}
    async with aiohttp.ClientSession() as session:
        async with session.get("http://localhost:5050/query", params=params) as resp:
            data = await resp.json()
...
async def test_5() -> None:
    ...
    params = {"assemblyId": "GRCh38", "referenceName": "MT", "start": 63, "referenceBases": "CT", "alternateBases": "NN", "includeDatasetResponses": "HIT"}
    async with aiohttp.ClientSession() as session:
        async with session.get("http://localhost:5050/query", params=params) as resp:
            data = await resp.json()
            if "datasetAlleleResponses" in data and len(data["datasetAlleleResponses"]) > 0:
...
does not seem correct because: ''referenceName' is a required property'"
    payload = {"start": 9, "referenceBases": "T", "alternateBases": "C", "assemblyId": "GRCh38", "includeDatasetResponses": "HIT"}
    async with aiohttp.ClientSession() as session:
        async with session.post("http://localhost:5050/query", data=json.dumps(payload)) as resp:
            data = await resp.json()
            if "error" in data and len(data["error"]) > 0:
